...
public extension JBLoading {
     ...
     */
    public func setCustomLoader(_ loader: UIView, startAnimationBlock start: @escaping ((Void) -> ()), stopAnimationBlock stop: @escaping ((Void) -> ())) {
     ...
     */
    fileprivate func prepareForLoading() {
    ...
    public func startLoading(withTitle title: String? = nil) {
        ...
        if let newTitle = title, newTitle.characters.count != 0 && !self.hideTitleOnLoad && self.title != nil {
    ...
    public func stopLoading(withTitle title: String? = nil) {
        ...
        if let newTitle = title, newTitle.characters.count != 0 && self.title != nil {
            self.setTitleText(newTitle)
        } else {
            self.setNeedsDisplay()
        }
